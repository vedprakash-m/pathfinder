...
class RealTimeCommunicationValidator:
    ...
    def validate_backend_websocket_endpoints(self):
        ...
        if not websocket_api_path.exists():
            self.log_result(
                "WebSocket API File",
                False,
                "websocket.py API file not found"
            )
            return
        ...
        try:
            ...
            self.log_result(
                ...
                trip_endpoint_found,
                "Trip WebSocket endpoint found in API" if trip_endpoint_found else "Missing trip WebSocket endpoint"
            )
            
            # Test 2: Notifications WebSocket endpoint
            notifications_endpoint_found = "/notifications" in content and "@router.websocket" in content
            self.log_result(
                "Notifications WebSocket Endpoint",
                notifications_endpoint_found,
                "Notifications WebSocket endpoint found" if notifications_endpoint_found else "Missing notifications WebSocket endpoint"
            )
            ...
            self.log_result(
                ...
                auth_handling_found,
                "WebSocket authentication handling found" if auth_handling_found else "Missing WebSocket authentication"
            )
            ...
            self.log_result(
                ...
                "Connection management found" if connection_mgmt_found else "Missing connection management"
            )
            ...
            self.log_result(
                ...
                False,
                f"Failed to analyze WebSocket API: {e}"
            )
    ...
    def validate_websocket_service_file(self):
        ...
        if not ws_service_path.exists():
            self.log_result(
                "WebSocket Service File",
                False,
                "websocket.py service file not found"
            )
            return
        ...
        try:
            ...
            self.log_result(
                ...
                "ConnectionManager class found" if connection_manager_found else "Missing ConnectionManager class"
            )
            
            # Test 2: Message handling
            message_handling_found = "handle_websocket_message" in content or "send_trip_message" in content
            self.log_result(
                "Message Handling",
                message_handling_found,
                "Message handling functions found" if message_handling_found else "Missing message handling"
            )
            
            # Test 3: Room management
            room_mgmt_found = "join_room" in content or "leave_room" in content or "broadcast_to_trip" in content
            self.log_result(
                "Room Management",
                room_mgmt_found,
            ...
            self.log_result(
                ...
                False,
                f"Failed to analyze WebSocket service: {e}"
            )
        ...
        try:
            ...
            for feature_name, found in features:
                self.log_result(
                    ...
                    found,
                    f"Frontend {feature_name.lower()} found" if found else f"Missing frontend {feature_name.lower()}"
                )
            ...
            self.log_result(
                ...
                False,
                f"Failed to analyze frontend WebSocket: {e}"
            )
        ...
        if hook_path.exists():
            try:
                ...
                self.log_result(
                    ...
                    hook_features_found,
                    "useWebSocket hook found" if hook_features_found else "Missing useWebSocket hook functionality"
                )
                ...
                self.log_result(
                    ...
                    False,
                    f"Failed to analyze WebSocket hook: {e}"
                )
    ...
    def validate_trip_chat_integration(self):
        ...
        if not chat_component_path.exists():
            self.log_result(
                "Trip Chat Component",
                False,
                "TripChat.tsx component not found"
            )
            return
        ...
        try:
            ...
            integration_features = [
                ("WebSocket Service Import", "webSocketService" in content or "websocketService" in content),
                ...
                ("Message Listeners", "webSocketService.on" in content or "websocketService.on" in content)
            ]
            ...
            for feature_name, found in integration_features:
                self.log_result(
                    f"Chat {feature_name}",
                    found,
                    f"Chat {feature_name.lower()} found" if found else f"Missing chat {feature_name.lower()}"
                )
            ...
            self.log_result(
                "Trip Chat Analysis",
                False,
                f"Failed to analyze trip chat: {e}"
            )
    ...
    async def validate_websocket_connectivity(self):
        ...
        try:
            ...
            try:
                ...
                trip_ws_url = f"ws://localhost:8000/ws/trip/test-trip-id?token=test-token"
                ...
                if "Connection refused" in str(e) or "connection was rejected" in str(e):
                    self.log_result(
                        "WebSocket Trip Endpoint",
                        False,
                        "Trip WebSocket endpoint not accessible"
                    )
                else:
                    self.log_result(
                        "WebSocket Trip Endpoint",
            ...
            try:
                notifications_ws_url = f"ws://localhost:8000/ws/notifications?token=test-token"
    ...
    def validate_real_time_feedback_service(self):
        ...
        if not feedback_service_path.exists():
            self.log_result(
                ...
                False,
                "real_time_feedback.py service not found"
            )
            return
        ...
        try:
            ...
            for feature_name, found in service_features:
                self.log_result(
                    ...
                    found,
                    f"Feedback {feature_name.lower()} found" if found else f"Missing feedback {feature_name.lower()}"
                )
    ...
    def validate_websocket_testing_infrastructure(self):
        ...
        self.log_result(
            ...
            "WebSocket tests found in backend" if websocket_test_found else "Missing WebSocket tests in backend"
        )
        ...
        self.log_result(
            ...
            "WebSocket E2E tests found" if e2e_websocket_test_found else "Missing WebSocket E2E tests"
        )
