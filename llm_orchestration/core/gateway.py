...
class LLMGateway:
    ...
    async def process_request(self, request: LLMRequest, tenant_info: TenantInfo) -> LLMResponse:
        """
    ...
    async def _validate_tenant_budget(self, request: LLMRequest, tenant_info: TenantInfo) -> None:
        ...
        budget_check = await self.budget_manager.check_budget_availability(
            tenant_info.tenant_id, request.user_id, estimated_cost
        )
    ...
    async def _process_with_circuit_breaker(
        ...
        try:
            ...
            adapter = await self.adapter_factory.get_adapter(provider_type, selected_model.model_id)
    ...
    async def _post_process_response(
        self, request: LLMRequest, response: LLMResponse, tenant_info: TenantInfo, start_time: float
    ) -> None:
        ...
        await self.budget_manager.record_usage(
            tenant_info.tenant_id, request.user_id, actual_cost, response.model_used
        )
    ...
    async def _log_cache_hit(
        self, request: LLMRequest, tenant_info: TenantInfo, start_time: float
    ) -> None:
        """Log cache hit for analytics"""
        processing_time = time.time() - start_time

        await self.analytics_collector.record_cache_hit(
            request=request, tenant_info=tenant_info, processing_time=processing_time
        )

    async def _handle_request_error(
        self, request: LLMRequest, tenant_info: TenantInfo, error: Exception, start_time: float
    ) -> None:
        ...
        await self.analytics_collector.record_error(
            request=request, tenant_info=tenant_info, error=error, processing_time=processing_time
        )
