...
from core.types import (
    LLMRequest, LLMProvider, TaskType, RequestPriority,
    LLMConfigurationError, ModelHealth
)
...
class RoutingRule:
    ...
    def matches(self, context: RoutingContext) -> bool:
        ...
        try:
            ...
            logger.error("Error evaluating routing condition", condition=self.condition, error=str(e))
            return False
...
class RoutingStrategyEngine:
    ...
    async def select_model(
        ...
        try:
            ...
            default_strategy = config.get("models", {}).get("routing_strategy", {}).get("default_strategy", "cost_optimized")
            ...
            if ab_assignment:
                ...
                logger.debug("Using A/B test assignment", user_id=request.user_id, assignment=ab_assignment)
            else:
    ...
    def _apply_user_preferences(self, candidate_models: List[str], request: LLMRequest) -> List[str]:
        ...
        active_models = [
            ...
            if self._model_definitions.get(m, {}).get("active", False)
        ]
    ...
    def _select_optimal_model(
        ...
        for model_id in available_models:
            score = self._calculate_model_score(
                model_id, context, budget_status, current_load
            )
    ...
    def _calculate_model_score(
        ...
        avg_cost = (
            model_def["cost_per_1k_tokens"]["input"] + 
            model_def["cost_per_1k_tokens"]["output"]
        ) / 2
        ...
        if health:
            ...
            response_time_score = max(0, 1.0 - (health.avg_response_time_ms / 10000))  # Penalty for slow responses
    ...
    async def get_fallback_models(
        ...
        try:
            ...
            fallback_models = [
                m for m in candidate_models 
                if m != original_model and m not in exclude_models
            ]
            ...
            logger.error("Failed to get fallback models", original_model=original_model, error=str(e))
            return []
