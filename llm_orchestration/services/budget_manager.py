...
class BudgetTracker:
    ...
    def _ensure_periods(self, timestamp: datetime):
        ...
        # Check daily period
        if self.current_daily_period is None or not self.current_daily_period.is_active(timestamp):
        ...
        if self.current_monthly_period is None or not self.current_monthly_period.is_active(
            timestamp
        ):
    ...
    def check_budget_available(self, estimated_cost: float, timestamp: datetime) -> bool:
        ...
        return (
            self.daily_usage + estimated_cost <= self.daily_limit
            and self.monthly_usage + estimated_cost <= self.monthly_limit
        )
...
class BudgetManager:
    ...
    async def _get_tenant_tracker(self, tenant_id: str) -> BudgetTracker:
        ...
        if tenant_id not in self.tenant_trackers:
            ...
            if "alert_thresholds" in tenant_defaults:
                self.tenant_trackers[tenant_id].alert_thresholds = tenant_defaults[
                    "alert_thresholds"
                ]
    ...
    async def enforce_budget(
        self, user_id: str, tenant_id: Optional[str], estimated_cost: float, model_id: str
    ) -> bool:
        """
    ...
    async def get_budget_status(
        self, user_id: str, tenant_id: Optional[str] = None, scope: str = "user"
    ) -> BudgetStatus:
        """
    ...
    async def get_usage_summary(
        self, user_id: Optional[str] = None, tenant_id: Optional[str] = None, hours: int = 24
    ) -> Dict[str, Any]:
        ...
        if user_id:
            ...
            # Global summary
            records.extend(
                [r for r in self.global_tracker.usage_records if r.timestamp >= cutoff_time]
            )
