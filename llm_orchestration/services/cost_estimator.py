...
class CostEstimator:
    ...
    async def calculate_actual_cost(self, request: LLMRequest, response: LLMResponse) -> float:
        """
        ...
        try:
            ...
            logger.warning(
                "Actual cost calculation failed", request_id=request.request_id, error=str(e)
            )
            return await self.estimate_request_cost(request)
    ...
    def _get_provider_pricing(
        self, provider: LLMProviderType, model_id: Optional[str] = None
    ) -> Dict[str, float]:
        """
    ...
    async def estimate_monthly_cost(
        ...
        cost_per_request = (estimated_input_tokens / 1000) * pricing["input_token_cost"] + (
    ...
    def get_current_pricing(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "default_pricing": {
                provider.value: pricing for provider, pricing in self.default_pricing.items()
            },
