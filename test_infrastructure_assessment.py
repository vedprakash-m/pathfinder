...
class TestInfrastructureQuickFix:
            ...
            try:
                ...
                for feature_name, found in config_features:
                    self.log_result(
                        ...
                        found,
                        f"Vitest {feature_name.lower()} configured" if found else f"Missing Vitest {feature_name.lower()}"
                    )
                ...
                self.log_result(
                    ...
                    False,
                    f"Failed to analyze Vitest config: {e}"
                )
        else:
            self.log_result(
                "Vitest Config File",
                ...
                try:
                    ...
                    for feature_name, found in setup_features:
                        self.log_result(
                            ...
                            found,
                            f"Test setup {feature_name.lower()} found" if found else f"Missing test setup {feature_name.lower()}"
                        )
    ...
    def run_quick_test_check(self):
        ...
        try:
            ...
            result = subprocess.run(
                ["npm", "test", "--", "--run", "--reporter=json", "src/tests/components/HomePage.test.tsx"],
            ...
            if result.returncode == 0:
                ...
                self.log_result(
                    "Sample Test Execution",
                    False,
                    f"Sample test failed: {result.stderr[:200]}..."
                )
            ...
            self.log_result(
                "Sample Test Execution",
                False,
                "Sample test timed out"
            )
        except Exception as e:
            self.log_result(
                "Sample Test Execution",
                False,
                f"Could not run sample test: {e}"
            )
    ...
    def check_backend_test_config(self):
        ...
        self.log_result(
            "Backend Test Config",
            pytest_config_found,
            "Backend test configuration found" if pytest_config_found else "Missing backend test configuration"
        )
            ...
            try:
                ...
                for feature_name, found in test_features:
                    self.log_result(
                        ...
                        found,
                        f"Backend {feature_name.lower()} found" if found else f"Missing backend {feature_name.lower()}"
                    )
                    
            except Exception:
                self.log_result(
                    "Backend Conftest Analysis",
                    False,
                    "Failed to analyze backend conftest"
                )
        else:
            self.log_result(
                "Backend Conftest File",
                False,
                "Backend conftest.py not found"
            )
    ...
    def provide_recommendations(self):
        ...
        if "Test Setup File" in failed_test_names:
            critical_issues.append("🔧 Create test setup file with proper DOM and mock configuration")
        ...
        if "Mock Directory" in failed_test_names:
            critical_issues.append("🔧 Create comprehensive mock directory for external dependencies")
            
        if "Sample Test Execution" in failed_test_names:
            critical_issues.append("🔧 Fix immediate test execution issues (likely mock/provider problems)")
