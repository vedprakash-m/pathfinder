        ...
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    print("✅ Consensus Analysis Endpoint: Working")
        ...
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                response = await client.get(
                    f"{self.base_url}/api/v1/coordination/status/{self.test_trip_id}"
                )

                if response.status_code == 200:
                    status_data = response.json()
                    health = status_data.get("automation_health", {})
                    print("✅ Coordination Status Endpoint: Working")
    ...
    async def test_feedback_integration(self):
        ...
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                response = await client.get(
                    f"{self.base_url}/api/v1/feedback/dashboard/{self.test_trip_id}"
                )

                if response.status_code == 200:
                    dashboard_data = response.json()
                    collab_health = dashboard_data.get("collaboration_health", {})
                    print("✅ Feedback Dashboard Endpoint: Working")
                    print(f"✅ Response Rate: {collab_health.get('response_rate', 'N/A')}%")
                    print(
                        f"✅ Avg Resolution Time: {collab_health.get('avg_resolution_time', 'N/A')}h"
                    )
                    return True
                else:
