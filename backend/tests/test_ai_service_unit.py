...
class TestAIServiceItineraryGeneration:
    ...
    @pytest.mark.asyncio
    async def test_generate_itinerary_success(self, ai_service, sample_trip_data, sample_preferences):
    ...
    async def test_generate_itinerary_api_error(self, mock_openai, ai_service, sample_trip_data, sample_preferences):
        ...
        with pytest.raises(Exception):
            await ai_service.generate_itinerary(sample_trip_data, sample_preferences)
            
    @patch('app.services.ai_service.openai')
    async def test_generate_itinerary_invalid_response(self, mock_openai, ai_service, sample_trip_data, sample_preferences):
...
class TestAIServiceRecommendations:
    ...
    async def test_get_restaurant_recommendations(self, mock_openai, ai_service):
            ...
            "restaurants": [
                {
                    ...
                    "recommended_dishes": ["Seasonal Vegetable Tasting", "Wild Mushroom Risotto"]
                }
            ]
        })
        ...
        result = await ai_service.get_restaurant_recommendations(location, dietary_restrictions, budget_level)
...
class TestAIServiceOptimization:
    ...
    async def test_optimize_budget_allocation(self, mock_openai, ai_service):
        ...
        result = await ai_service.optimize_budget_allocation(total_budget, trip_duration, family_count)
...
class TestAIServiceErrorHandling:
    ...
    async def test_rate_limit_handling(self, mock_openai, ai_service):
        ...
        mock_openai.chat.completions.create = AsyncMock(side_effect=RateLimitError("Rate limit exceeded"))
