...
class TestTripIntegration:
    ...
    async def test_complete_trip_workflow(self):
        ...
        async with AsyncClient(app=app, base_url="http://test") as client:
            ...
            assert delete_response.status_code in [status.HTTP_200_OK, status.HTTP_204_NO_CONTENT, status.HTTP_404_NOT_FOUND]
...
class TestFamilyManagementIntegration:
    ...
    async def test_family_creation_and_management(self):
        ...
        async with AsyncClient(app=app, base_url="http://test") as client:
            ...
            create_response = await client.post("/api/v1/families", json=family_data, headers=headers)
            if create_response.status_code == 201:
                ...
                update_response = await client.put(f"/api/v1/families/{family_id}", json=update_data, headers=headers)
    ...
    async def test_trip_participant_consistency(self):
        ...
        async with AsyncClient(app=app, base_url="http://test") as client:
            ...
            if join_response.status_code == 200:
                ...
                trip_details_response = await client.get(f"/api/v1/trips/{trip_id}", headers=headers)
                ...
                participants_response = await client.get(f"/api/v1/trips/{trip_id}/participants", headers=headers)
