...
def upgrade() -> None:
    ...
    op.alter_column('itinerary_activities', 'google_place_id',
               existing_type=sa.VARCHAR(length=255),
               type_=sa.String(length=100),
               existing_nullable=True)
    op.alter_column('reservations', 'google_place_id',
               existing_type=sa.VARCHAR(length=255),
               type_=sa.String(length=100),
               existing_nullable=True)
    op.alter_column('reservations', 'duration_hours',
               existing_type=sa.INTEGER(),
               type_=sa.Numeric(precision=5, scale=2),
               existing_nullable=True)
    op.alter_column('reservations', 'cancellation_policy',
               existing_type=sa.VARCHAR(length=14),
               type_=sa.Enum('FREE_CANCELLATION', 'MODERATE', 'STRICT', 'NO_REFUND', name='cancellationpolicy'),
               existing_nullable=True)
    op.alter_column('reservations', 'external_provider',
    ...
    op.alter_column('users', 'role',
               ...
               type_=sa.Enum('SUPER_ADMIN', 'FAMILY_ADMIN', 'TRIP_ORGANIZER', 'FAMILY_MEMBER', name='userrole'),
               existing_nullable=False,
...
def downgrade() -> None:
    ...
    op.alter_column('users', 'phone',
               ...
               existing_nullable=True)
    op.alter_column('users', 'role',
               existing_type=sa.Enum('SUPER_ADMIN', 'FAMILY_ADMIN', 'TRIP_ORGANIZER', 'FAMILY_MEMBER', name='userrole'),
               type_=sa.VARCHAR(length=20),
               existing_nullable=False,
               existing_server_default=sa.text("'family_admin'"))
    op.alter_column('users', 'auth0_id',
    ...
    op.alter_column('reservations', 'external_provider',
               ...
               existing_nullable=True)
    op.alter_column('reservations', 'cancellation_policy',
               existing_type=sa.Enum('FREE_CANCELLATION', 'MODERATE', 'STRICT', 'NO_REFUND', name='cancellationpolicy'),
