...
class Day2MigrationValidator:
    ...
    def generate_day2_report(self) -> str:
        ...
        sql_cleanup_pct = (sum(self.results['sql_cleanup'].values()) / 
                          len(self.results['sql_cleanup']) * 100) if self.results['sql_cleanup'] else 0
        
        cosmos_functionality_pct = (sum(self.results['cosmos_functionality'].values()) / 
                                   len(self.results['cosmos_functionality']) * 100) if self.results['cosmos_functionality'] else 0
        
        endpoint_migration_pct = (sum(self.results['endpoint_migration'].values()) / 
                                 len(self.results['endpoint_migration']) * 100) if self.results['endpoint_migration'] else 0
