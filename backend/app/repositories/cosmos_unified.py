...
class UnifiedCosmosRepository:
    ...
    def _ensure_container_exists(self):
        ...
        try:
            ...
            self.database.create_container_if_not_exists(
                ...
                offer_throughput=None  # Use serverless
            )
    ...
    async def get_user_trips(
        ...
        """ + (f" AND c.status = @status" if status_filter else "") + """
        ORDER BY c.created_at DESC
    ...
    async def update_trip(self, trip_id: str, trip_update: Any) -> TripDocument:
        ...
        # Apply updates
        update_data = trip_update.dict(exclude_unset=True) if hasattr(trip_update, 'dict') else trip_update
    ...
    async def delete_trip(self, trip_id: str) -> None:
        ...
        try:
            ...
            AND c.trip_id = @trip_id
            """
            message_results = await self._query_documents(messages_query, [{"name": "@trip_id", "value": trip_id}])
            ...
            logger.info(f"Successfully deleted trip {trip_id} and {len(message_results)} related messages")
    ...
    async def create_family_invitation(self, invitation_data: Dict[str, Any]) -> FamilyInvitationDocument:
        """Create a new family invitation."""
        invitation_doc = FamilyInvitationDocument(
            pk=f"family_{invitation_data['family_id']}",
            **invitation_data
        )
    ...
    async def get_family_invitation_by_token(self, token: str) -> Optional[FamilyInvitationDocument]:
    ...
    async def update_family_invitation(self, invitation_id: str, update_data: Dict[str, Any]) -> Optional[FamilyInvitationDocument]:
    ...
    async def get_trip_messages(self, trip_id: str, room_id: Optional[str] = None, limit: int = 50) -> List[MessageDocument]:
        """Get messages for a trip."""
    ...
    async def create_reservation(self, reservation_data: Dict[str, Any]) -> ReservationDocument:
        ...
        reservation_doc = ReservationDocument(
            pk=f"trip_{reservation_data['trip_id']}",
            **reservation_data
        )
        
        await self._create_document(reservation_doc)
        logger.info(f"Created reservation: {reservation_doc.id} for trip: {reservation_data['trip_id']}")
        return reservation_doc

    async def get_trip_reservations(self, trip_id: str, status: Optional[str] = None) -> List[ReservationDocument]:
    ...
    async def update_reservation(self, reservation_id: str, updates: Dict[str, Any]) -> Optional[ReservationDocument]:
    ...
    async def create_feedback(self, feedback_data: Dict[str, Any]) -> FeedbackDocument:
        ...
        feedback_doc = FeedbackDocument(
            pk=f"trip_{feedback_data['trip_id']}",
            **feedback_data
        )
    ...
    async def get_trip_feedback(self, trip_id: str, status: Optional[str] = None) -> List[FeedbackDocument]:
    ...
    async def update_feedback(self, feedback_id: str, updates: Dict[str, Any]) -> Optional[FeedbackDocument]:
    ...
    async def create_export_task(self, export_data: Dict[str, Any]) -> ExportDocument:
        ...
        export_doc = ExportDocument(
            pk=f"user_{export_data['user_id']}",
            **export_data
        )
    ...
    async def get_user_exports(self, user_id: str, status: Optional[str] = None) -> List[ExportDocument]:
    ...
    async def update_export_task(self, export_id: str, updates: Dict[str, Any]) -> Optional[ExportDocument]:
    ...
    async def create_itinerary(self, itinerary_data: Dict[str, Any]) -> ItineraryDocument:
        ...
        itinerary_doc = ItineraryDocument(
            pk=f"trip_{itinerary_data['trip_id']}",
            **itinerary_data
        )
    ...
    async def get_trip_itineraries(self, trip_id: str, active_only: bool = True) -> List[ItineraryDocument]:
    ...
    async def update_itinerary(self, itinerary_id: str, updates: Dict[str, Any]) -> Optional[ItineraryDocument]:
    ...
    async def _get_document(self, partition_key: str, item_id: str, model_class) -> Optional[CosmosDocument]:
    ...
    async def _query_documents(self, query: str, parameters: List[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        ...
        if not self.container:
            ...
            for pk_data in self._simulation_data.values():
                for doc in pk_data.values():
                    ...
                    if "entity_type" in query and "'user'" in query and doc.get("entity_type") == "user":
                        results.append(doc)
                    elif "entity_type" in query and "'family'" in query and doc.get("entity_type") == "family":
                        results.append(doc)
                    elif "entity_type" in query and "'trip'" in query and doc.get("entity_type") == "trip":
                        results.append(doc)
                    elif "entity_type" in query and "'message'" in query and doc.get("entity_type") == "message":
                        results.append(doc)
            return results
