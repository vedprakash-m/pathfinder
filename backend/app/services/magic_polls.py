...
from app.models.ai_integration import (
    MagicPoll, PollType, PollStatus, create_magic_poll
)
...
class MagicPollsService:
    ...
    async def submit_response(
        ...
        try:
            ...
            poll.responses["user_responses"] = [
                r for r in poll.responses["user_responses"] 
    ...
    async def get_trip_polls(
        ...
        try:
            polls = db.query(MagicPoll).filter(
                MagicPoll.trip_id == trip_id
            ).order_by(MagicPoll.created_at.desc()).all()
        ...
        try:
            ...
            if vote_counts:
                ...
                return {
                    ...
                    "confidence": "high" if consensus_strength > 0.7 else "moderate" if consensus_strength > 0.5 else "low",
