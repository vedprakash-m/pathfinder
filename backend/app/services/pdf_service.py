...
class PDFService:
    ...
    def _create_title_page(self, trip: TripDetail, itinerary: Itinerary) -> List[Any]:
        ...
        # Title
        title = Paragraph(f"Trip Itinerary: {trip.name}", self.styles.get('CustomTitle', self.styles.get('Title')))
        elements.append(title)
        elements.append(Spacer(1, 20))
        
        # Destination
        destination = Paragraph(f"Destination: {trip.destination}", self.styles.get('CustomHeader', self.styles.get('Heading2')))
        ...
        duration_text = Paragraph(f"Duration: {duration} days", self.styles.get('CustomBody', self.styles.get('Normal')))
        elements.append(duration_text)
        elements.append(Spacer(1, 30))
        
        # Description
        if trip.description:
            desc_title = Paragraph("Trip Description", self.styles.get('CustomSubHeader', self.styles.get('Heading3')))
            elements.append(desc_title)
            description = Paragraph(trip.description, self.styles.get('CustomBody', self.styles.get('Normal')))
    ...
    def _create_trip_overview(self, trip: TripDetail) -> List[Any]:
        ...
        # Section header
        header = Paragraph("Trip Overview", self.styles.get('CustomHeader', self.styles.get('Heading2')))
        ...
        if hasattr(trip, 'participations') and trip.participations:
            participants_header = Paragraph("Participants", self.styles.get('CustomSubHeader', self.styles.get('Heading3')))
            ...
            for participation in trip.participations:
                ...
                participant = Paragraph(participant_text, self.styles.get('CustomBody', self.styles.get('Normal')))
    ...
    def _create_daily_itinerary(
        ...
        # Section header
        header = Paragraph("Daily Itinerary", self.styles.get('CustomHeader', self.styles.get('Heading2')))
        elements.append(header)
        elements.append(Spacer(1, 12))
        ...
        for day in sorted(itinerary_days, key=lambda d: d.day_number):
            ...
            if day.theme:
                theme = Paragraph(f"Theme: {day.theme}", self.styles.get('CustomBody', self.styles.get('Normal')))
            ...
            if day_activities:
                for activity in sorted(day_activities, key=lambda a: a.start_time or datetime.min.time()):
                    activity_elements = self._create_activity_entry(activity)
                    elements.extend(activity_elements)
            else:
                no_activities = Paragraph("No activities scheduled for this day.", self.styles.get('CustomBody', self.styles.get('Normal')))
    ...
    def _create_activity_entry(self, activity: ItineraryActivity) -> List[Any]:
        ...
        for detail in details:
            detail_para = Paragraph(f"  {detail}", self.styles.get('CustomBody', self.styles.get('Normal')))
    ...
    def _create_important_info(self, trip: TripDetail) -> List[Any]:
        ...
        # Section header
        header = Paragraph("Important Information", self.styles.get('CustomHeader', self.styles.get('Heading2')))
        elements.append(header)
        elements.append(Spacer(1, 12))
        
        # Emergency contacts
        emergency_header = Paragraph("Emergency Contacts", self.styles.get('CustomSubHeader', self.styles.get('Heading3')))
        ...
        # Travel tips
        tips_header = Paragraph("Travel Tips", self.styles.get('CustomSubHeader', self.styles.get('Heading3')))
    ...
    def generate_trip_summary_pdf(self, trip: TripDetail, reservations: List[Reservation] = None) -> bytes:
        """
        ...
        try:
            ...
            # Title
            title = Paragraph(f"Trip Summary: {trip.name}", self.styles.get('CustomTitle', self.styles.get('Title')))
            story.append(title)
    ...
    def _create_reservations_summary(self, reservations: List[Reservation]) -> List[Any]:
        ...
        # Section header
        header = Paragraph("Reservations", self.styles.get('CustomHeader', self.styles.get('Heading2')))
        elements.append(header)
        elements.append(Spacer(1, 12))
        ...
        for res_type, res_list in grouped_reservations.items():
            type_header = Paragraph(res_type.title(), self.styles.get('CustomSubHeader', self.styles.get('Heading3')))
    ...
    def _create_reservation_entry(self, reservation: Reservation) -> List[Any]:
        ...
        # Reservation name
        name = Paragraph(f"• {reservation.name}", self.styles.get('CustomBody', self.styles.get('Normal')))
    ...
    def generate_family_roster_pdf(self, families: List[Family]) -> bytes:
        ...
        try:
            ...
            # Title
            title = Paragraph("Family Roster", self.styles.get('CustomTitle', self.styles.get('Title')))
            story.append(title)
            story.append(Spacer(1, 20))
            
            for family in families:
                # Family name
                family_header = Paragraph(family.name, self.styles.get('CustomHeader', self.styles.get('Heading2')))
                ...
                if hasattr(family, 'members') and family.members:
                    for member in family.members:
                        ...
                        member_para = Paragraph(member_info, self.styles.get('CustomBody', self.styles.get('Normal')))
