...
class CostMonitoringService:
    ...
    async def send_cost_alert(self, service: str, percentage: float, threshold: float) -> None:
        ...
        if not self.notification_service:
            logger.warning(f"Cost alert triggered but notification service not available: {service} at {percentage:.1f}%")
            return
...
class AIModelSelector:
    ...
    async def select_model(
        ...
        if quality_requirement == "high" and task_type in ["complex_reasoning", "detailed_planning"]:
            return "gpt-4o"
