...
class LLMOrchestrationClient:
    ...
    def __init__(self):
        ...
        self.client = httpx.AsyncClient(
            timeout=httpx.Timeout(self.timeout), headers=self._get_headers(), follow_redirects=True
        )
    ...
    def _get_headers(self) -> Dict[str, str]:
        ...
        headers = {"Content-Type": "application/json", "User-Agent": "Pathfinder-Backend/1.0.0"}
    ...
    async def generate_text(
        ...
        try:
            ...
            response = await self.client.post(
                f"{self.base_url}/api/v1/llm/process", json=request_data
            )
            ...
            if response.status_code == 200:
                ...
                logger.error(
                    f"LLM Orchestration HTTP error: {response.status_code} - {response.text}"
                )
    ...
    async def get_analytics(
        self, tenant_id: str = "pathfinder-main", hours: int = 24
    ) -> Dict[str, Any]:
        ...
        try:
            response = await self.client.get(
                f"{self.base_url}/api/v1/analytics/tenant/{tenant_id}", params={"hours": hours}
            )
