...
class AnalyticsService:
    ...
    async def track_event(
        ...
        try:
            ...
            logger.info(f"Analytics event tracked: {event_name}", 
                       user_id=user_id, event_type=event_type.value)
        ...
        try:
            ...
            for event in events:
                if event['event_type'] == EventType.USER_ACTION.value:
                    action_counts[event['event_name']] = action_counts.get(event['event_name'], 0) + 1
        ...
        try:
            ...
            for feature, stats in feature_stats.items():
                ...
                adoption_metrics[feature] = {
                    ...
                    'adoption_rate': unique_user_count / total_unique_users if total_unique_users > 0 else 0,
                }
        ...
        try:
            ...
            return {
                ...
                "summary": {
                    ...
                    "avg_response_time": performance_summary.get('api_response_time', {}).get('avg', 0)
                }
            }
    ...
    async def get_business_metrics(
        ...
        try:
            ...
            if 'trip_completed' in metrics and 'trip_started' in metrics:
                ...
                business_summary['trip_completion_rate'] = completed_trips / started_trips if started_trips > 0 else 0
            
            # User satisfaction scores
            if 'user_satisfaction' in metrics:
                satisfaction_scores = [m['value'] for m in metrics['user_satisfaction'] if isinstance(m['value'], (int, float))]
                if satisfaction_scores:
                    business_summary['avg_satisfaction'] = sum(satisfaction_scores) / len(satisfaction_scores)
                    business_summary['satisfaction_responses'] = len(satisfaction_scores)
            
            # Time saved metrics
            if 'time_saved_hours' in metrics:
                time_saved_values = [m['value'] for m in metrics['time_saved_hours'] if isinstance(m['value'], (int, float))]
                if time_saved_values:
                    business_summary['total_time_saved_hours'] = sum(time_saved_values)
                    business_summary['avg_time_saved_per_trip'] = sum(time_saved_values) / len(time_saved_values)
    ...
    async def _get_events_by_type(self, event_type: EventType, hours: int) -> List[Dict[str, Any]]:
        ...
        while current_date <= end_date.date():
            ...
            for event in day_events:
                if (event.get('event_type') == event_type.value and 
                    start_date <= datetime.fromisoformat(event['timestamp']) <= end_date):
                    events.append(event)
