...
class DataExportService:
    ...
    async def export_trip_data_excel(
        ...
        try:
            ...
            if include_participants and trip_data.get('participations'):
                excel_data['Participants'] = self._prepare_participants_data(trip_data['participations'])
        ...
        try:
            ...
            if export_type in ["participants", "all"]:
                if trip_data.get('participations'):
                    csv_files['participants'] = self._create_participants_csv(trip_data['participations'])
    ...
    def _prepare_participants_data(self, participations: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
            ...
            for day_data in daily_itinerary:
                ...
                for activity in day_activities:
                    activities.append({
                        ...
                        "Description": activity.get('description', '')[:100] + "..." if len(activity.get('description', '')) > 100 else activity.get('description', '')
                    })
