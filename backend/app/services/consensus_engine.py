...
class FamilyConsensusEngine:
    ...
    def generate_weighted_consensus(
        ...
        family_weights = self.calculate_family_weights(
        ...
        next_steps = self._generate_next_steps(
            consensus_score, conflicts, voting_items)
        ...
        for category, family_prefs in preference_categories.items():
            ...
            if len(unique_values) > 1:
                ...
                conflict = PreferenceConflict(
                    ...
                    severity=severity,
                    suggested_compromise=self._suggest_compromise(
                        category, values),
                )
    ...
    def _calculate_conflict_severity(
        ...
        high_categories = ["budget_level",
            "accommodation_type", "transportation_mode"]
    ...
    def _weighted_preference_calculation(
        ...
        # For categorical values (e.g., budget_level, travel_style)
            ...
            for value, weight in zip(values, weights):
                value_str = str(value)
                value_weights[value_str] = value_weights.get(
                    value_str, 0) + weight
    ...
    def _generate_next_steps(
        ...
        if consensus_score >= 0.8:
            ...
            next_steps.append(
                "⚠️ Moderate consensus - address remaining conflicts")
            if voting_items:
                next_steps.append(
                    f"🗳️ {len(voting_items)} items need family votes")
            next_steps.append(
                "💬 Schedule family discussion to resolve differences")

        else:
            next_steps.append(
                "❌ Low consensus - significant discussion needed")
            next_steps.append("📞 Schedule video call with all families")
            next_steps.append(
                "🔄 Consider revising preferences after discussion")
    ...
    def get_consensus_dashboard_data(
        ...
        consensus_result = self.generate_weighted_consensus(
            families_data, total_budget)
        family_weights = self.calculate_family_weights(
            families_data, total_budget)
...
def _generate_trip_recommendations(consensus_result: ConsensusResult) -> List[str]:
    ...
    if score >= 0.8:
        ...
        recommendations.append(
            "Send summary to families for final confirmation")
        ...
        recommendations.append(
            "Consider compromise solutions for disputed preferences")
        ...
        recommendations.append(
            "Focus on finding common ground in 2-3 key areas")
