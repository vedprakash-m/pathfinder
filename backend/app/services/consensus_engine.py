...
class FamilyConsensusEngine:
    ...
    def generate_weighted_consensus(self, families_data: List[Dict[str, Any]], 
                                   total_budget: float) -> ConsensusResult:
    ...
    def calculate_family_weights(self, families_data: List[Dict[str, Any]], 
                                total_budget: float) -> List[FamilyWeight]:
        ...
        for family in families_data:
            ...
            budget_contribution = budget_allocation / total_budget if total_budget > 0 else 1.0 / len(families_data)
    ...
    def detect_preference_conflicts(self, families_data: List[Dict[str, Any]]) -> List[PreferenceConflict]:
    ...
    def _calculate_conflict_severity(self, category: str, values: List[Any]) -> ConflictSeverity:
        ...
        critical_categories = ["accessibility_needs", "dietary_restrictions", "medical_requirements"]
    ...
    def _suggest_compromise(self, category: str, values: List[Any]) -> str:
        ...
        if category == "budget_level":
            ...
            common_activities = [item for item, count in Counter(all_activities).items() if count > 1]
    ...
    def _calculate_weighted_preferences(self, families_data: List[Dict[str, Any]], 
                                      family_weights: List[FamilyWeight]) -> Dict[str, Any]:
    ...
    def _weighted_preference_calculation(self, category: str, values: List[Any], 
    ...
    def _calculate_consensus_score(self, conflicts: List[PreferenceConflict], 
                                 family_count: int) -> float:
    ...
    def _generate_voting_items(self, conflicts: List[PreferenceConflict]) -> List[Dict[str, Any]]:
        ...
        for i, conflict in enumerate(conflicts):
            ...
            if conflict.severity in [ConflictSeverity.MEDIUM, ConflictSeverity.HIGH, ConflictSeverity.CRITICAL]:
                ...
                voting_item = VotingItem(
                    ...
                    options=conflict.conflicting_values + [conflict.suggested_compromise] if conflict.suggested_compromise else conflict.conflicting_values,
                    current_votes={},
    ...
    def _generate_ai_compromises(self, conflicts: List[PreferenceConflict], 
                               weighted_prefs: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _generate_next_steps(self, consensus_score: float, conflicts: List[PreferenceConflict], 
                           voting_items: List[Dict[str, Any]]) -> List[str]:
    ...
    def process_family_vote(self, voting_item_id: str, family_id: str, 
                           vote_choice: str, voting_items: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    def get_consensus_dashboard_data(self, families_data: List[Dict[str, Any]], 
        ...
        dashboard_data = {
            ...
            "conflict_summary": {
                ...
                "by_severity": {
                    severity.value: len([c for c in consensus_result.conflicts if c.severity == severity])
                    for severity in ConflictSeverity
                },
                "critical_items": [c.category for c in consensus_result.conflicts 
                                 if c.severity == ConflictSeverity.CRITICAL]
            },
            "next_actions": consensus_result.next_steps,
            "voting_summary": {
                "pending_votes": len([v for v in consensus_result.voting_items 
                                    if v.get("status") == VoteStatus.PENDING.value]),
                "items_needing_discussion": len([v for v in consensus_result.voting_items 
...
def analyze_trip_consensus(trip_id: str, families_data: List[Dict[str, Any]], 
                          total_budget: float) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(f"Consensus analysis completed for trip {trip_id}. Score: {consensus_result.consensus_score:.2f}")
...
def _generate_trip_recommendations(consensus_result: ConsensusResult) -> List[str]:
    ...
    if score >= 0.8:
        ...
        high_priority_conflicts = [c for c in conflicts if c.severity in [ConflictSeverity.HIGH, ConflictSeverity.CRITICAL]]
        if high_priority_conflicts:
            recommendations.append(f"Priority: Address {len(high_priority_conflicts)} high-impact conflicts")
