...
class NotificationService:
    ...
    async def send_admin_alert(self, alert_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            alert_notification = NotificationCreate(
                ...
                priority=NotificationPriority.HIGH if alert_data.get("severity") == "high" else NotificationPriority.MEDIUM,
            ...
            logger.info(f"Admin alert sent to {len(admin_users)} administrators", alert_type=alert_data.get("type"))
            return True
    ...
    async def send_cost_threshold_alert(self, service: str, current_usage: float, threshold: float, percentage: float):
    ...
    async def _send_email_notification(self, notification: Notification, response: NotificationResponse) -> None:
        """Send email notification based on type and user preferences."""
        try:
            ...
            if not await self._should_send_email_notification(str(notification.user_id), notification.type):
                logger.debug(f"Email notifications disabled for user {notification.user_id}, type {notification.type}")
                return
            
            # Send email based on notification type
            success = False
            if notification.type == NotificationType.TRIP_INVITATION:
                success = await self._send_trip_invitation_email(notification, user_email, user_name)
            elif notification.type == NotificationType.ITINERARY_READY:
                success = await self._send_itinerary_ready_email(notification, user_email, user_name)
            ...
            if success:
                ...
                logger.warning(f"Failed to send email notification for {notification.type} to {user_email}")
    ...
    async def _should_send_email_notification(self, user_id: str, notification_type: NotificationType) -> bool:
        """Check if user has email notifications enabled for this type."""
    ...
    async def _send_trip_invitation_email(self, notification: Notification, user_email: str, user_name: str) -> bool:
    ...
    async def _send_itinerary_ready_email(self, notification: Notification, user_email: str, user_name: str) -> bool:
    ...
    async def _send_trip_update_email(self, notification: Notification, user_email: str, user_name: str) -> bool:
    ...
    async def _send_system_alert_email(self, notification: Notification, user_email: str, user_name: str) -> bool:
