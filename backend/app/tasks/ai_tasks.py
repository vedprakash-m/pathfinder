...
def generate_itinerary_async(self, trip_id: str, preferences: Dict[str, Any], user_id: str):
    ...
    try:
        ...
        return run_async_task(_generate_itinerary_async(trip_id, preferences, user_id))
    except Exception as exc:
        logger.error(f"Itinerary generation failed for trip {trip_id}: {exc}")
        # Retry with exponential backoff (only if Celery is available)
    ...
    async for db in get_db():
        try:
            ...
            for participation in trip.participations:
                ...
                family_data = {"name": family.name, "members": []}
...
@conditional_task(bind=True, name="app.tasks.ai_tasks.optimize_itinerary_async")
...
@conditional_task(name="app.tasks.ai_tasks.generate_daily_cost_report")
def generate_daily_cost_report():
    """Generate daily AI cost usage report."""
    try:
        return run_async_task(_generate_daily_cost_report())
    except Exception as exc:
    ...
    try:
        ...
        if today in cost_tracker.daily_usage:
            ...
            if usage_data.get("cost", 0) > settings.AI_DAILY_BUDGET_LIMIT * 0.8:
                logger.warning(
                    f"AI costs approaching daily budget limit: {usage_data.get('cost', 0)}"
                )
