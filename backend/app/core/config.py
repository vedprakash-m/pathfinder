...
class UnifiedSettings(BaseSettings):
    ...
    ALLOWED_HOSTS: Union[List[str], str] = Field(default=["localhost", "127.0.0.1"], description="Allowed host headers")
    ...
    DATABASE_TIMEOUT: int = Field(default=30, ge=5, le=300, description="Database connection timeout in seconds")
    ...
    COSMOS_DB_CONTAINER_ITINERARIES: str = Field(default="itineraries", description="Itineraries container name")
    COSMOS_DB_CONTAINER_MESSAGES: str = Field(default="messages", description="Messages container name")
    COSMOS_DB_CONTAINER_PREFERENCES: str = Field(default="preferences", description="Preferences container name")
    ...
    JWT_EXPIRATION: int = Field(default=3600, ge=300, le=86400, description="JWT expiration time in seconds")
    ...
    OPENAI_MAX_TOKENS: int = Field(default=2000, ge=100, le=8000, description="Maximum tokens per request")
    ...
    RATE_LIMIT_REQUESTS: int = Field(default=100, ge=10, le=10000, description="Rate limit requests per window")
    RATE_LIMIT_WINDOW: int = Field(default=3600, ge=60, le=86400, description="Rate limit window in seconds")
    
    # File upload security
    MAX_FILE_SIZE: int = Field(default=10*1024*1024, ge=1024, le=100*1024*1024, description="Maximum file size in bytes")
    ...
    # Context validation
    ENABLE_CONTEXT_VALIDATION: bool = Field(default=False, description="Enable security context validation")
    MAX_RISK_THRESHOLD: float = Field(default=0.7, ge=0.0, le=1.0, description="Maximum security risk threshold")
    MFA_TRIGGER_THRESHOLD: float = Field(default=0.5, ge=0.0, le=1.0, description="MFA trigger threshold")
    ...
    AZURE_STORAGE_CONTAINER: str = Field(default="uploads", description="Azure Storage container name")
    
    # Application Insights
    APPINSIGHTS_CONNECTION_STRING: Optional[str] = Field(default=None, description="Application Insights connection string")
    
    # ==================== PERFORMANCE MONITORING ====================
    SLOW_REQUEST_THRESHOLD: float = Field(default=1.0, ge=0.1, le=60.0, description="Slow request threshold in seconds")
    SLOW_QUERY_THRESHOLD: float = Field(default=0.5, ge=0.1, le=30.0, description="Slow query threshold in seconds")
    METRICS_ROLLUP_INTERVAL: int = Field(default=300, ge=60, le=3600, description="Metrics rollup interval in seconds")
    
    # ==================== AI COST TRACKING ====================
    AI_COST_TRACKING_ENABLED: bool = Field(default=True, description="Enable AI cost tracking")
    AI_DAILY_BUDGET_LIMIT: float = Field(default=50.0, ge=1.0, le=1000.0, description="Daily AI budget limit in USD")
    
    # ==================== WEBSOCKET CONFIGURATION ====================
    WEBSOCKET_HEARTBEAT_INTERVAL: int = Field(default=30, ge=10, le=300, description="WebSocket heartbeat interval")
    WEBSOCKET_MAX_CONNECTIONS: int = Field(default=1000, ge=10, le=10000, description="Maximum WebSocket connections")
    
    # ==================== LLM ORCHESTRATION ====================
    LLM_ORCHESTRATION_ENABLED: bool = Field(default=False, description="Enable LLM orchestration service")
    LLM_ORCHESTRATION_URL: Optional[str] = Field(default=None, description="LLM orchestration service URL")
    LLM_ORCHESTRATION_API_KEY: Optional[str] = Field(default=None, description="LLM orchestration API key")
    LLM_ORCHESTRATION_TIMEOUT: int = Field(default=60, ge=10, le=300, description="LLM orchestration timeout")
    ...
    WORKING_HOURS_START: int = Field(default=8, ge=0, le=23, description="Working hours start (24h format)")
    WORKING_HOURS_END: int = Field(default=18, ge=0, le=23, description="Working hours end (24h format)")
    WORKING_HOURS_TIMEZONE: str = Field(default="UTC", description="Working hours timezone")
    USER_PATTERNS_FILE: str = Field(default="user_access_patterns.json", description="User patterns storage file")
    SLOW_FUNCTION_THRESHOLD: float = Field(default=0.5, ge=0.1, le=30.0, description="Slow function threshold")
    ...
    def validate_database_configuration(self):
        ...
        if self.USE_REDIS_CACHE and not self.REDIS_URL:
            logger.warning("Redis cache is enabled but REDIS_URL is not provided - falling back to memory cache")
    ...
    def validate_external_services(self):
        ...
        if self.SENDGRID_API_KEY and any([self.SMTP_HOST, self.SMTP_USERNAME]):
            logger.warning("Both SendGrid and SMTP configurations detected - SendGrid will take precedence")
        ...
        if self.AZURE_STORAGE_ACCOUNT and not (self.AZURE_STORAGE_KEY or self.AZURE_STORAGE_CONNECTION_STRING):
            logger.warning("Azure Storage account specified but no key or connection string provided")
...
def get_settings() -> UnifiedSettings:
    ...
    logger.info(f"Configuration loaded successfully for environment: {settings_instance.ENVIRONMENT}")
