...
class RateLimitStore:
    ...
    def add_request(self, key: str, endpoint: str) -> Tuple[int, int]:
        ...
        self._store[key] = {
            req_time: ts for req_time, ts in self._store[key].items()
    ...
    def _cleanup(self):
        ...
        for key in list(self._store.keys()):
            self._store[key] = {
                req_id: ts for req_id, ts in self._store[key].items()
                if now - ts < self.window_size
            }
...
class RateLimiter(BaseHTTPMiddleware):
    ...
    def _should_skip(self, request: Request) -> bool:
        ...
        if hasattr(request, "client") and request.client and (
            request.client.host in ("testclient", "testserver", "127.0.0.1")
        ):
            return True
