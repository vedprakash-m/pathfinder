    ...
    def get_usage_stats(self, user_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        if user_id and user_id in today_data:
            return {
                ...
                'remaining': {
                    'user_budget': max(0, self.cost_thresholds['user_limit'] - today_data[user_id]['cost'])
                }
            }
...
class AICostManagementMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        logger.info(f"AI request processed: {request.url} - User: {user_id} - Time: {processing_time:.2f}s")
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            if 'current_user' in kwargs:
                ...
                user_id = user_data.get('id', 'anonymous') if isinstance(user_data, dict) else getattr(user_data, 'id', 'anonymous')
            ...
            if user_id != 'anonymous' and 'remaining' in usage_stats:
                ...
                user_budget = remaining.get('user_budget', remaining.get('daily_budget', 10.0))  # fallback to daily or default
            ...
            try:
                ...
                if isinstance(result, dict):
                    result['ai_usage'] = {
                        ...
                        'remaining_budget': usage_stats.get('remaining', {}).get('user_budget', 
