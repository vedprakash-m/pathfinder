...
class AuthenticationMonitoringMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if is_auth_endpoint:
            ...
            if not auth_header or not auth_header.startswith('Bearer '):
                ...
                logger.info('Authentication attempt without token', extra={
    ...
    def _update_auth_metrics(self, response: Response, duration: float):
        ...
        total_auth_requests = self.auth_metrics['authenticated_requests'] + self.auth_metrics['failed_auth_requests']
        if total_auth_requests > 0:
            current_avg = self.auth_metrics['avg_auth_duration']
            self.auth_metrics['avg_auth_duration'] = (
                (current_avg * (total_auth_requests - 1) + duration) / total_auth_requests
            )
