...
@asynccontextmanager
async def monitoring_context(operation_name: str, **context) -> AsyncGenerator[OperationMetrics, None]:
    ...
    try:
        ...
        # Record metrics
        await collector.record_timer(
            f"block.{operation_name.replace(' ', '_').lower()}",
            metrics.duration_ms
        )
...
class HealthChecker:
    ...
    def get_overall_health(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_updated": max(
                (r.get("timestamp", "") for r in self.last_results.values()),
                default=""
            )
        }
