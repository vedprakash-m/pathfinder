...
async def _maybe_rollup_metrics():
    ...
    now = datetime.now()
    if (now - _metrics_store["last_metrics_rollup"]).total_seconds() > settings.METRICS_ROLLUP_INTERVAL:
...
async def _rollup_metrics():
    ...
    _metrics_store["memory_usage_samples"] = [
        sample for sample in _metrics_store["memory_usage_samples"]
        if sample[0] >= cutoff
    ]
