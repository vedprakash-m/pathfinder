...
def extract_standard_user(token_claims: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if 'roles' in token_claims:
        ...
        role_to_permissions = {
            ...
            'FamilyMember': [
                'read:trips', 'create:trips', 'update:trips',
                'read:families', 'update:families',
                'read:itineraries', 'create:itineraries', 'update:itineraries'
            ],
            'User': [
                'read:trips', 'read:families', 'read:itineraries'
            ]
        }
        ...
        permissions = [
            'read:trips', 'create:trips', 'update:trips',
            'read:families', 'create:families', 'update:families',
            'read:itineraries', 'create:itineraries', 'update:itineraries'
        ]
    ...
    if 'ved_preferences' in token_claims:
        try:
            ved_preferences = json.loads(token_claims['ved_preferences']) if isinstance(token_claims['ved_preferences'], str) else token_claims['ved_preferences']
    ...
    return {
        ...
        'name': token_claims.get('name') or token_claims.get('preferred_username') or token_claims['email'],
