...
class PermissionDeniedError(AuthenticationError):
    ...
    def __init__(self, required_permission: str = None, details: Dict[str, Any] = None):
        message = f'Insufficient permissions. Required: {required_permission}' if required_permission else None
...
def create_auth_error_response(error: AuthenticationError, status_code: int = status.HTTP_401_UNAUTHORIZED) -> HTTPException:
    """Create standardized HTTPException for authentication errors."""
    
    # Log authentication error for monitoring
    logger.warning(f"Authentication error: {error.code} - {error.message}", extra={
...
def handle_auth_error(error: Exception) -> HTTPException:
    ...
    if isinstance(error, jwt.ExpiredSignatureError):
        ...
        return create_auth_error_response(
            TokenInvalidError({'reason': str(error)})
        )
