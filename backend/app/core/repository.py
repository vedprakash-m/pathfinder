...
class SqlAlchemyRepository(Repository[T]):
    ...
    async def update(self, id: str, updates: Dict[str, Any], 
                    return_updated: bool = True) -> Optional[T]:
        ...
        try:
            ...
            for field, value in filters.items():
                if isinstance(value, dict):
                    ...
                    if "in" in value:
                        conditions.append(f"c.{field} IN ({','.join([f'@{field}_{i}' for i in range(len(value['in']))])})")
...
class RepositoryFactory:
    ...
    def __init__(self, db_session_factory, cosmos_containers: Optional[Dict[str, ContainerProxy]] = None):
    ...
    def create_cosmos_repository(self, container_name: str, partition_key_field: str = "id") -> CosmosDbRepository:
    ...
    def decorator(repository_method):
        async def wrapper(*args, **kwargs):
            ...
            try:
                ...
                logger.info(f"Repository operation completed", extra={
                ...
                duration = (time.time() - start_time) * 1000
                logger.error(f"Repository operation failed", extra={
                    "operation": operation_name,
                    "method": repository_method.__name__,
                    "duration_ms": duration,
                    "success": False,
