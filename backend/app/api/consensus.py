...
async def get_consensus_dashboard(
    ...
    try:
        ...
        dashboard_data = {
            ...
            "status": "Strong Consensus" if consensus_result.consensus_score >= 0.8 else "Needs Discussion",
            "family_count": len(families_data),
            "conflicts_summary": {
                "total": len(consensus_result.conflicts),
                "critical": len([c for c in consensus_result.conflicts if c.severity.value == "critical"]),
...
async def submit_family_vote(
    ...
    try:
        ...
        logger.info(f"Vote submitted for trip {trip_id} by family {user_family_id}: {vote_request.vote_choice}")
...
async def get_consensus_recommendations(
    ...
    try:
        ...
        recommendations = {
            ...
            "consensus_score": consensus_result.consensus_score,
            "priority_conflicts": [
                asdict(c) for c in consensus_result.conflicts 
...
async def _check_trip_access(db: AsyncSession, trip_id: str, user_id: str) -> bool:
    ...
    try:
        ...
        participation_stmt = select(TripParticipation).join(Family).where(
            TripParticipation.trip_id == trip_id,
            Family.admin_user_id == user_id
        )
...
async def _get_user_family_for_trip(db: AsyncSession, trip_id: str, user_id: str) -> Optional[str]:
    ...
    try:
        stmt = select(Family.id).join(TripParticipation).where(
            TripParticipation.trip_id == trip_id,
            Family.admin_user_id == user_id
        )
        result = await db.execute(stmt)
