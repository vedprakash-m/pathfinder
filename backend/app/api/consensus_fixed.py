...
async def create_consensus(
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=f"Failed to create consensus: {str(e)}")
...
async def get_consensus_results(
    ...
    try:
        ...
        for option_idx, option in enumerate(consensus_doc["options"]):
            vote_count = sum(1 for vote in consensus_doc["responses"].values() 
