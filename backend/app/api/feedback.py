    ...
    try:
        ...
        if response_request.new_status:
            try:
                ...
                raise HTTPException(status_code=400, detail=f"Invalid status: {response_request.new_status}")
...
async def submit_live_change(
    ...
    try:
        ...
        if result["success"]:
            ...
            raise HTTPException(status_code=400, detail=result.get("error", "Failed to submit live change"))
