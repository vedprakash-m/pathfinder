...
async def test_family_invitation_workflow():
    ...
    try:
        ...
        try:
            ...
            print(f"   📬 Email send attempted: {'✅ Success' if email_result else '⚠️  No email service configured'}")
        ...
        stored_invitation = db.query(FamilyInvitationModel).filter(
            FamilyInvitationModel.invitation_token == invitation_token
        ).first()
        ...
        final_invitation = db.query(FamilyInvitationModel).filter(
            FamilyInvitationModel.invitation_token == invitation_token
        ).first()
        
        print(f"✅ Invitation status: {final_invitation.status.value}")
        if final_invitation.accepted_at:
            print(f"   📅 Accepted at: {final_invitation.accepted_at.strftime('%Y-%m-%d %H:%M:%S')}")
