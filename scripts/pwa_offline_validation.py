...
class PWAOfflineValidator:
    ...
    def validate_pwa_manifest(self):
        ...
        if not manifest_path.exists():
            self.log_result(
                "PWA Manifest File", False, "manifest.json not found in public directory"
            )
            return
        ...
        try:
            ...
            if not missing_fields:
                ...
                self.log_result(
                    "Manifest Required Fields", False, f"Missing required fields: {missing_fields}"
                )
            ...
            self.log_result(
                ...
                has_required_icons,
                (
                    f"Required icon sizes found: {required_sizes}"
                    if has_required_icons
                    else f"Missing required icon sizes: {required_sizes}"
                ),
            )
            ...
            self.log_result(
                ...
                is_app_like,
                (
                    f"App-like display mode: {display_mode}"
                    if is_app_like
                    else f"Browser-like display mode: {display_mode}"
                ),
            )

        except Exception as e:
            self.log_result("Manifest File Analysis", False, f"Failed to analyze manifest: {e}")
    ...
    def validate_service_worker(self):
        ...
        self.log_result("Service Worker File", True, f"Service Worker found at {sw_file}")
        ...
        try:
            ...
            for feature_name, found in cache_features:
                self.log_result(
                    ...
                    found,
                    (
                        f"Service Worker {feature_name.lower()} found"
                        if found
                        else f"Missing Service Worker {feature_name.lower()}"
                    ),
                )

        except Exception as e:
            self.log_result(
                "Service Worker Analysis", False, f"Failed to analyze Service Worker: {e}"
            )
    ...
    def validate_sw_registration(self):
        ...
        self.log_result(
            ...
            (
                f"Service Worker registration found in {registration_file}"
                if sw_registration_found
                else "Service Worker registration not found in app"
            ),
        )
    ...
    def validate_offline_components(self):
        ...
        self.log_result(
            ...
            (
                "Offline page/component found"
                if offline_component_found
                else "Missing offline page/component"
            ),
        )
        ...
        self.log_result(
            ...
            (
                "Network status detection found"
                if network_detection_found
                else "Missing network status detection"
            ),
        )
    ...
    def validate_offline_storage(self):
        ...
        self.log_result(
            ...
            storage_found,
            (
                f"Offline storage found: {', '.join(set(storage_types))}"
                if storage_found
                else "No offline storage implementation found"
            ),
        )
        ...
        if not caching_lib_found:
            self.log_result("Client-side Caching", False, "No client-side caching libraries found")
    ...
    def validate_background_sync(self):
        ...
        self.log_result(
            ...
            (
                "Background sync capability found"
                if background_sync_found
                else "No background sync implementation found"
            ),
        )
    ...
    def validate_offline_fallbacks(self):
        ...
        self.log_result(
            ...
            (
                "API offline error handling found"
                if offline_error_handling
                else "Missing API offline error handling"
            ),
        )
        ...
        try:
            for file_path in self.src_path.rglob("*.tsx"):
                ...
                if "offline" in content.lower() and (
                    "fallback" in content.lower() or "cache" in content.lower()
                ):
        ...
        self.log_result(
            ...
            offline_first_found,
            (
                "Offline-first components found"
                if offline_first_found
                else "No offline-first components found"
            ),
        )
    ...
    def validate_pwa_meta_tags(self):
        ...
        if not html_path.exists():
            self.log_result("HTML File", False, "index.html not found")
            return
        ...
        try:
            ...
            pwa_meta_tags = [
                ...
                (
                    "Apple Touch Icon",
                    'rel="apple-touch-icon"' in html_content or "apple-touch-icon" in html_content,
                ),
                (
                    "App Capable",
                    'name="mobile-web-app-capable"' in html_content
                    or 'name="apple-mobile-web-app-capable"' in html_content,
                ),
            ]
            ...
            self.log_result(
                "HTML Meta Tags Analysis", False, f"Failed to analyze HTML meta tags: {e}"
            )
    ...
    def validate_build_configuration(self):
        ...
        if vite_config_path.exists():
            try:
                ...
                self.log_result("Vite Config Analysis", False, "Failed to analyze Vite config")
        ...
        if package_json_path.exists():
            try:
                ...
                if found_pwa_deps:
                    ...
                    self.log_result(
                        "PWA Build Dependencies", False, "No PWA build dependencies found"
                    )

            except Exception:
                self.log_result("Package.json Analysis", False, "Failed to analyze package.json")
